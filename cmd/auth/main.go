...
type config struct {
	...
	AccessDuration    time.Duration `env:"MG_AUTH_ACCESS_TOKEN_DURATION"   envDefault:"30m"`
...
func newService(db *sqlx.DB, tracer trace.Tracer, cfg config, dbConfig pgclient.Config, logger mglog.Logger, spicedbClient *authzed.Client) auth.Service {
	...
	svc := auth.New(keysRepo, idProvider, t, pa, cfg.AccessDuration, cfg.RefreshDuration)
