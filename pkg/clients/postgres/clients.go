...
func (repo ClientRepository) RetrieveByID(ctx context.Context, id string) (clients.Client, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return clients.Client{}, errors.Wrap(repoerror.ErrNotFound, err)
		}
		return clients.Client{}, errors.Wrap(repoerror.ErrViewEntity, err)
	}
...
func (repo ClientRepository) RetrieveAll(ctx context.Context, pm clients.Page) (clients.ClientsPage, error) {
	...
	if err != nil {
		return clients.ClientsPage{}, errors.Wrap(repoerror.ErrViewEntity, err)
	}
...
func (repo ClientRepository) update(ctx context.Context, client clients.Client, query string) (clients.Client, error) {
	...
	if err != nil {
		return clients.Client{}, errors.Wrap(repoerror.ErrUpdateEntity, err)
	}

	row, err := repo.DB.NamedQueryContext(ctx, query, dbc)
	if err != nil {
		return clients.Client{}, postgres.HandleError(err, repoerror.ErrUpdateEntity)
	}

	defer row.Close()
	if ok := row.Next(); !ok {
		return clients.Client{}, errors.Wrap(repoerror.ErrNotFound, row.Err())
	}
	dbc = DBClient{}
	if err := row.StructScan(&dbc); err != nil {
...
func ToDBClient(c clients.Client) (DBClient, error) {
	...
	if len(c.Metadata) > 0 {
		...
		if err != nil {
			return DBClient{}, errors.Wrap(repoerror.ErrMalformedEntity, err)
		}
		data = b
	}
...
func ToClient(c DBClient) (clients.Client, error) {
	...
	if c.Metadata != nil {
		if err := json.Unmarshal([]byte(c.Metadata), &metadata); err != nil {
			return clients.Client{}, errors.Wrap(repoerror.ErrMalformedEntity, err)
		}
	}
	var tags []string
...
func toDBClientsPage(pm clients.Page) (dbClientsPage, error) {
	...
	if err != nil {
		return dbClientsPage{}, errors.Wrap(repoerror.ErrViewEntity, err)
	}
...
func pageQuery(pm clients.Page) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(repoerror.ErrViewEntity, err)
	}
	var query []string
	var emq string
	if mq != "" {
