...
func TestCreateClient(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			response: sdk.User{},
			token:    token,
			err:      errors.NewSDKErrorWithStatus(errors.Wrap(apiutil.ErrValidation, repoerror.ErrMalformedEntity), http.StatusBadRequest),
		},
		{
...
func TestListClients(t *testing.T) {
	...
	for _, tc := range cases {
		...
		repoCall1 := auth.On("CheckAdmin", mock.Anything, mock.Anything).Return(svcerror.ErrAuthorization)
